...
class Router {
  ...
  dispatch(url, context) {
    ...
    const route = this.routes.find(
      (route) => (matches = urlSplit[0].match(route.matcher))
    );
...
function createCone(coneConfig) {
        ...
        const replaceContent = (content) => {
          if (typeof content === "string") {
            ...
            return routerElement.replaceChildren(
              domParser.parseFromString(content, "text/html").body.firstChild
            );
          } else if (content instanceof HTMLElement) {
            return routerElement.replaceChildren(content);
          } else {
            return routerElement.replaceChildren(
              content(_params, _query, _context)
            );
          }
        };
  ...
  function link(props, ...children) {
    const { name, target, params, query, navState, preload, ...otherProps } =
      props;
